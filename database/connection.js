...
async function initializeDatabase() {
  try {
    ...
    console.log(
      "🚂 Railway Environment:",
      process.env.RAILWAY_ENVIRONMENT || "false"
    );
    ...
    inMemoryData.users = [
      {
        ...
        password_hash:
          "$2b$10$N9qo8uLOickgx2ZMRZoMye/hgcAlQe7GUJl7G6iEWpKXpMLOG3.h2", // demo123
        first_name: "Demo",
        last_name: "User",
        role: "user",
        is_active: true,
    ...
    try {
      ...
      if (query.includes("FROM products")) {
        ...
        if (params.length > 0) {
          ...
          console.log(
            `🎯 Filtered by domain '${domain}': ${results.length} products`
          );
        }

        // Search functionality
        if (query.includes("LIKE")) {
          const searchTerm = params.find(
        ...
        if (query.includes("LIMIT")) {
          ...
          if (limitMatch) {
            ...
            console.log(
              `📄 Pagination: limit=${limit}, offset=${offset}, results=${results.length}`
            );
          }
        }
      ...
      if (query.includes("FROM categories")) {
        ...
        if (params.length > 0) {
          ...
          console.log(
            `📂 Categories for domain '${domain}': ${results.length}`
          );
        }
        return results;
      }
    ...
    try {
      ...
      if (query.includes("FROM products")) {
        ...
        if (query.includes("COUNT(*)")) {
          const domain = params[0];
          const count = inMemoryData.products.filter(
            (p) => p.domain === domain
          ).length;
        ...
        if (query.includes("WHERE") && params.length >= 2) {
          const domain = params[0];
          const id = params[1];
          const result = inMemoryData.products.find(
            (p) => p.domain === domain && p.id == id
          );
          console.log(
            `🎯 Product domain='${domain}', id=${id}:`,
      ...
      if (query.includes("FROM users")) {
        if (query.includes("WHERE username =")) {
          ...
          const result = inMemoryData.users.find(
  ...
  executeQuery: (query, params = []) => {
    try {
      ...
      if (query.includes("SELECT DISTINCT domain FROM products")) {
        const domains = [
          ...new Set(inMemoryData.products.map((p) => p.domain)),
        ];
      ...
      if (
        query.includes("FROM categories c") &&
        query.includes("LEFT JOIN products p")
      ) {
        const domain = params[0];
        const categories = inMemoryData.categories.filter(
          (c) => c.domain === domain
        );
        const result = categories.map((category) => {
          const productCount = inMemoryData.products.filter(
            (p) => p.domain === domain && p.category_id === category.id
          ).length;
          return { ...category, product_count: productCount };
        });
        console.log(
          `📂 Categories with count for '${domain}': ${result.length}`
        );
        return result;
      }
      ...
      if (
        ...
        const result = brands.map((brand) => {
          const productCount = inMemoryData.products.filter(
            (p) => p.domain === domain && p.brand_id === brand.id
          ).length;
      ...
      if (
        ...
        query.includes("LEFT JOIN categories c")
      ) {
        const domain = params[0];
        ...
        const result = products.map((product) => {
          const category = inMemoryData.categories.find(
            (c) => c.domain === domain && c.id === product.category_id
          );
          const brand = inMemoryData.brands.find(
            (b) => b.domain === domain && b.id === product.brand_id
          );
        ...
        if (query.includes("LIMIT")) {
          ...
          console.log(
            `🛍️ Products for '${domain}': ${paginatedResult.length}/${result.length}`
          );
